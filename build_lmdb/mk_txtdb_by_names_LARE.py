...
def process_jsonl(jsonf, db, toker, max_tokens=100, dataset_name="", filter_path=None, filter_path_val=None, \
                include_path=None, num_samples=0, corpus_name='msp'):
    '''
    {
        "segmentId": [
...
def main(opts):
    ...
    with open_db() as db:
        id2lens, txt2img, img2txt = process_jsonl(opts.input, db, toker, dataset_name=opts.dataset_name, \
                                ...
                                include_path=opts.include_path, num_samples=opts.num_samples, corpus_name=opts.corpus_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--corpus_name', required=True,
                        help='which corpus_name, iemocap, msp')
