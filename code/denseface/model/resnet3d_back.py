...
'''
Conv3D+ ResNet18,  https://github.com/afourast/deep_lip_reading
...
class ResNet3D(nn.Module):
    def __init__(self, gpu_id, block_type, resblocks, num_classes=8, conv3d=[5,3,3], zero_init_residual=False,
        ...
        # input: (1,64,64) -- first conv3d, kernel-size, (5,3,3) or (3,3,3)
        # 如果是30/25FPS那么用533，如果是10FPS那么用333.
        self.features = nn.Sequential(OrderedDict([
            ("conv0", nn.Conv3d(1, self.inplanes, kernel_size=(3,3,3), stride=(1,2,2), padding=(1,1,1), bias=False)),
            ("norm0", nn.BatchNorm3d(self.inplanes, momentum=0.01, eps=0.001)),
            ("relu0", nn.ReLU(inplace=True)),
            ("pool0", nn.MaxPool3d(kernel_size=(1,2,2), stride=(1,2,2), padding=(0,1,1))) 
        ]))
        if block_type == 'basic':
