...
class UniterForImageTextRetrievalHardNeg(UniterForImageTextRetrieval):
    ...
    def forward(self, batch, sample_from='t', compute_loss=True):
        ...
        if sample_from == 't':
            ...
            if img_position_ids.size(0) == 1:
                batch['img_position_ids'] = img_position_ids.expand(batch_size, -1)
        else:
            raise ValueError()
    ...
    def _get_hard_batch(self, batch, scores, sample_from='t'):
        ...
        if img_position_ids.size(0) != 1:
            img_position_ids = img_position_ids[:self.hard_size+1]
        ...
        hard_batch['img_position_ids'] = img_position_ids
