...
class SpeechEncoderBertModel(BertPreTrainedModel):
    ...
    def forward(self, batch, output_all_encoded_layers=False):
        ...
        # print(f'[Debug] affined a_output {affine_a_output.shape}') # torch.Size([1, seq-len/8, 768])
        ...
        attention_mask = torch.ones((affine_a_output.size(0), affine_a_output.size(1)), device=affine_a_output.device)
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
        extended_attention_mask = extended_attention_mask.to(
                                    dtype=next(self.parameters()).dtype)  # fp16 compatibility
        # print('[Debug] extended_attention_mask {}'.format(extended_attention_mask.shape))

        position_ids = torch.arange(0, affine_a_output.size(1), dtype=torch.long, device=affine_a_output.device).unsqueeze(0)
