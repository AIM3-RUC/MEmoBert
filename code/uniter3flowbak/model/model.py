...
from code.uniter3flow.model.enc_speech_new import SpeechWav2vecEncoderBertModel
...
def get_gather_index(txt_lens, num_bbs, batch_size, max_len, out_size):
    '''
    txt=13, img=10, 目前 txt-max=20, img-max=20, pad 之后的数据是
...
class MEmoBertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, use_speech, use_visual, pretrained_text_checkpoint=None):
        ...
        if use_speech:
            if self.s_config.use_wav2vec:
                logger.info('[Info] use the wav2vec speech encoder')
                self.speech_encoder = SpeechEncoderBertModel(self.s_config)
            else:
                logger.info('[Info] use the conv1d+transformer speech encoder')
                self.speech_encoder = SpeechWav2vecEncoderBertModel(self.s_config)
