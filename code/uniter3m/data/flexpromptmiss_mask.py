...
---- 针对模态缺失设计，不同的模态缺失场景，给予不同的标记。
ws = toker2.tokenize('L V S :')
[1048, 1058, 1055, 1024]
"""
import random
import torch
...
class FlexPromptMissMaskDataset(DetectFeatTxtTokDataset):
    def __init__(self, txt_db, img_db, speech_db, use_text, prompt_type):
        # 因此txt_db是必须的，所以得用 use_text 来表示是否有 text 信息
        ...
        self.set_prompt_input_ids = {
            ...
            'halfsoftprompt5iam': [[2], [3], [4], [5], [6], [1045], [2572], [103]],
        }
        task_names = {'L': [[1048],[1024]], 'LV': [[1048],[1058],[1024]], 'LS': [[1048],[1055],[1024]], 'LVS':[[1048],[1058],[1055],[1024]], 
                                'VS':[[1058],[1055],[1024]], 'V': [[1058],[1024]], 'S': [[1055],[1024]]}
        if self.use_text:
            if img_db is None and speech_db is None:
                self.prompt_task_name_ids = task_names['L']
            elif img_db is None and speech_db is not None:
                self.prompt_task_name_ids = task_names['LS']
            elif img_db is not None and speech_db is None:
                self.prompt_task_name_ids = task_names['LV']
            elif img_db is not None and speech_db is not None:
                self.prompt_task_name_ids = task_names['LVS']
            else:
                print('Error in FlexPromptMissMaskDataset')
        else:
            if img_db is None and speech_db is not None:
                self.prompt_task_name_ids = task_names['S']
            elif img_db is not None and speech_db is None:
                self.prompt_task_name_ids = task_names['V']
            elif img_db is not None and speech_db is not None:
                self.prompt_task_name_ids = task_names['VS']
            else:
                print('Error in FlexPromptMissMaskDataset')
        
        self.prompt_input_ids = self.prompt_task_name_ids + self.set_prompt_input_ids[prompt_type]
    ...
    def __getitem__(self, i):
        ...
        if self.use_text:
            input_ids, txt_labels = self.create_mlm_io_withL(example['input_ids'], self.prompt_input_ids, example['target'])
        else:
            input_ids, txt_labels = self.create_mlm_io_noL(self.prompt_input_ids, example['target'])
    ...
    def create_mlm_io_withL(self, input_ids, prompt_input_ids, target_value):
    ...
    def create_mlm_io_noL(self, prompt_input_ids, target_value):
...
def flexpromptmiss_mask_collate(inputs):
    """
    Return:
