...
def main(opts):
    ...
    cls_features = np.asarray(cls_features)
    targets = np.asarray(targets)
    np.save(os.path.join(opts.output_dir, 'label.npy'), targets)
    np.save(os.path.join(opts.output_dir, 'cls_fts.npy'), cls_features)
    # Manually check the samples' length 
    print('Features extracted with shape: {}, with target: {}'.format(cls_features.shape, targets.shape))
    for i in range(5):
        if len(txt_features) > 0:
