...
from code.uniter3m_mae.model.model import UniterModel, UniterPreTrainedModel, UniterEncoder
from code.uniter3m_mae.model.lare_layers import BertPostagHead, BertSentiHead, BertPolarityHead
## from uniter 
...
class UniterMAEForPretraining(UniterPreTrainedModel):
    """ UNITER pretraining """
    def __init__(self, config, img_dim, speech_dim, img_label_dim, use_visual, use_speech):
        ...
        self.uniter = Uniter(config, img_dim, speech_dim, use_visual=self.use_visual, 
        ...
        # for MAE decoding
        if (self.use_visual or self.use_speech):
            print('[Debug] use MAEDecoder for visual and audio feature reconstruction')
    ...
    def forward(self, batch, task, compute_loss=True):
        ...
        if task.startswith('mlm') or task.startswith('promptmask') or task.startswith('cmpromptmask'):
        ...
        elif task == 'itm' or task == 'vtm' or task == 'stm' or task == 'eitm' or task == 'onemodalnegitm' or task == 'promptnsp':
            ...
            img_masks = batch['img_masks']
            mrfr_feat_target = batch['feat_targets']
            return self.forward_mae_mrfr(batch, img_masks, img_mask_tgt,
                                     mrfr_feat_target, compute_loss=compute_loss)
        elif task == 'mae_msrfr':
            speech_mask_tgt = batch['speech_mask_tgt']
            speech_masks = batch['speech_masks']
            msrfr_feat_target = batch['feat_targets']
            return self.forward_mae_msrfr(batch, speech_masks, speech_mask_tgt,
                                     msrfr_feat_target, compute_loss=compute_loss)
        else:
...
class UniterForPretraining(UniterPreTrainedModel):
    ...
    def __init__(self, config, img_dim, speech_dim, img_label_dim, use_visual, use_speech):
        ...
        ## lrc add: 如果需要用MAE添加MAE模块,直接使用UniterEncoder结构
        if self.use_mae:
            print('[Debug] use MAE like decoder for feature regression!!!')
            self.mae_decoder = UniterEncoder(config.decoder_conf)
    ...
    def forward(self, batch, task, compute_loss=True):
        ...
        ## lrc add: 添加MAE decoder预测缺损语音视觉帧信息任务
        elif task == 'mae_mrfr':
            img_mask_tgt = batch['img_mask_tgt']
            img_masks = batch['img_masks']
            mrfr_feat_target = batch['feat_targets']
    ...
    ## lrc add: 添加MAE decoder预测缺损视觉帧信息
    def forward_mae_mrfr(self, batch, img_masks, img_mask_tgt,
                     feat_targets, compute_loss=True):

        sequence_output = self.uniter(batch, output_all_encoded_layers=False,
                                      img_masks=img_masks)
        # only compute masked tokens for better efficiency
        decoded_output = self.decoder(sequence_output, batch['attn_masks'])
        masked_output = self._compute_masked_hidden(decoded_output,
                                                    img_mask_tgt)
        prediction_feat = self.feat_regress(masked_output)
    ...
    ## lrc add: 添加MAE decoder预测缺损语音帧信息
    def forward_mae_msrfr(self, batch, speech_masks, speech_mask_tgt,
                     feat_targets, compute_loss=True):
